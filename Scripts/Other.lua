...
function ModeLoop(self)
    self:linear((53 + self:GetY()) / 75)
    self:y(-53);
    self:sleep(0)
    self:addy(150)
    self:queuecommand('Loop')
end
function ModeLoop2(self)
    self:decelerate((53 + self:GetY()) / 75)
    self:y(-53);
    self:sleep(0)
    self:addy(150)
    self:queuecommand('Loop')
end
function ModeColor(self)
    local a = self:getaux();
    a = math.mod(a + 6, 12)
    self:aux(a)
    if self:GetZ() == 0 then self:diffuse(ColorRGB(a)) end
end
function ModeColorOn(self)
    self:z(0)
...
function ModeColorOff(self)
    self:diffuse(0.20, 0.24, 0.26, 1)
    self:z(1)
end
...
function DifficultyListCommand(self, t)
    i = self:getaux()
    self:y((i - 1) * (19.3))
    self:shadowlength(0)
    if t == 'meter' then
    ...
    --self, self:getaux(), 'meter'
    DifficultyListRow(self, i, t)
end

function TitleMenuOut(self)
    self:sleep(.2)
    self:linear(.5)
    self:diffusealpha(0)
end
function OutCommand(self)
    self:linear(.5)
    self:diffusealpha(0)
end
...
function Style(n)
    local k = n
    local list = {
        "single", "versus", "double", "solo", "single", "versus", "double",
        "halfdouble", "single8", "versus8"
    }
    if game == "pump" then
        k = k + 4
...
function SelectButtonAvailable() return true end

function ScreenEndingGetDisplayName(pn)
    if PROFILEMAN:IsPersistentProfile(pn) then
        return GAMESTATE:GetPlayerDisplayName(pn)
    end
    return "No Card"
end
...
function StopCourseEarly()
    ...
    for pn = PLAYER_1, NUM_PLAYERS - 1 do
        if GAMESTATE:IsPlayerEnabled(pn) and not GAMESTATE:IsGoalComplete(pn) then
            return "0"
        end
    end
    return "1"
...
function SetDifficultyFrameFromSteps(Actor, pn)
    ...
    if steps then Actor:setstate(steps:GetDifficulty()) end
end
...
function GetInitialDifficulty() return "beginner" end

function DifficultyChangingIsAvailable()
    return GAMESTATE:GetPlayMode() ~= PLAY_MODE_ENDLESS and
               GAMESTATE:GetPlayMode() ~= PLAY_MODE_ONI and
               GAMESTATE:GetSortOrder() ~= SORT_MODE_MENU
end
...
function Get2PlayerJoinMessage()
    ...
    if GAMESTATE:GetCoinMode() == COIN_MODE_FREE then
        return "2 Player mode available"
    end

    local numSidesNotJoined = NUM_PLAYERS - GAMESTATE:GetNumSidesJoined()
    if GAMESTATE:GetPremium() == PREMIUM_JOINT then
        numSidesNotJoined = numSidesNotJoined - 1
    end
    local coinsRequiredToJoinRest = numSidesNotJoined *
                                        PREFSMAN:GetPreference("CoinsPerCredit")
...
function NonCombos()
    ...
    t.SaveSelections = function(self, list, pn)
        if list[1] then
            SetPref('JudgeWindowSecondsGood', 0.135)
            SetPref('JudgeWindowSecondsBoo', 0.180)
        end
        if list[2] then
            SetPref('JudgeWindowSecondsGood', 0.135)
            SetPref('JudgeWindowSecondsBoo', 0.135)
        end
        if list[3] then
            SetPref('JudgeWindowSecondsGood', 0.102)
            SetPref('JudgeWindowSecondsBoo', 0.102)
        end
    end
    return t
end
...
function Decents()
    if math.abs(PREFSMAN:GetPreference('JudgeWindowSecondsGood') - 0.135) < .001 then
        return true
    end
end
function WayOffs()
    if math.abs(PREFSMAN:GetPreference('JudgeWindowSecondsBoo') - 0.180) < .001 then
        return true
    end
end
