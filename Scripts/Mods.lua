...
function ScreenList(str)
    if type(screenList[str]) == 'function' then
        return screenList[str]()
    else
        return screenList[str]
    end
end
...
-- Judgment Font List
judgmentFontList = {
    'Default', 'Love', 'Tactics', 'Chromatic', 'Deco', 'GrooveNights', 'ITG2'
}
if FUCK_EXE then -- Auto load on NotITG
    ...
    for _, v in pairs({
        GAMESTATE:GetFileStructure('Themes/' .. dir .. '/Graphics/_Judgments/')
    }) do
        local t, _, name = string.find(v, "(.+) %dx%d")
        if t then
            table.insert(list, name)
        else
            print('[Judgment] Error in loading ' .. v)
        end
    end
...
function CompareTextColor(n)
    if n < 0 then return 1, .3, 1, 1 end
    return 0.3, 1, 0.3, 1
end
...
playerOptions = {}
playerOptions[1] = {
    'SpeedType', 'SpeedNumber', 'Mini', 'Perspective', 'NoteSkin', 'Turn',
    'LifeBar', 'Compare', 'Rate'
}
playerOptions[2] = {
    'MetaMods1', 'MetaMods2', 'MetaMods3', 'Turn', 'Accel', 'Scroll', 'Effect',
    'Appearance', 'Handicap', 'InsertTaps', 'InsertOther', 'Hide', 'Ghost'
}
playerOptions.Edit = {
    'SpeedType', 'SpeedNumber', 'Mini', 'Perspective', 'NoteSkin', 'Turn'
}
...
-----------------------
function number(n)
    return tonumber(tostring(n)) or loadstring('return ' .. n)() or 0
end
...
function Path(ec, str)
    return THEME:GetPath(_G['EC_' .. string.upper(ec)], '', str)
end
local function Player(pn) return GAMESTATE:IsPlayerEnabled(pn - 1) end
function PlayerIndex(pn)
    if pn == GAMESTATE:GetNumPlayersEnabled() then return pn end
    return 1
end
function Profile(pn)
...
function ThemeFile(file)
    return THEME:GetPath(EC_GRAPHICS, '', themeDir .. '/' .. file)
end
function ThemeName()
    local str = string.sub(THEME:GetPath(2, '', '_blank.png'), 9)
    return string.sub(str, 1, string.find(str, '/') - 1)
end
function VocalizePath()
    return '/Themes/' ..
               tostring(Profile(0).Love and Profile(0).Love.Dir or 'Simply Love') ..
               '/Vocalize/'
end
...
function GetStartScreen()
    SetPref("DelayedScreenLoad", false)
    if GetPref('BreakComboToGetItem') and GetInputType and GetInputType() == "" then
...
function MaxLength(str, l)
    if string.len(str) > l then str = string.sub(str, 0, l - 3) .. '...' end
    return str
end
function RowMetric(b, a, r)
    if r then
        rowYNum = 0
        rowYAdd = a
        rowYBase = b
        rowYOffTop = rowYBase + rowYAdd * 0.5
        return r
    elseif a then
        rowYNum = rowYNum + a
    end
    rowYNum = rowYNum + 1
    if b ~= 'Exit' then
        rowYOffCenter = rowYBase + rowYAdd *
                            (rowYNum + 1 + math.mod(rowYNum, 2)) / 2
        rowYOffBottom = rowYBase + rowYAdd * (rowYNum + 1 / 2)
    end
    return rowYBase + rowYAdd * rowYNum
end
function SecondsToMSS(n)
    local t = SecondsToMSSMsMs(math.abs(n))
    t = string.sub(t, 0, string.len(t) - 3)
    if tonumber(n) < 0 then t = '-' .. t end
    return t
end
function MSSMsMsToSeconds(t)
    return string.sub(t, string.len(t) - 4, string.len(t)) +
               string.sub(t, 1, string.len(t) - 6) * 60
end
...
function Diffuse(self, c, n)
    if not c[4] then c[4] = 1 end
    if n == 1 then
        self:diffuseupperleft(c[1], c[2], c[3], c[4])
    elseif n == 2 then
        self:diffuseupperright(c[1], c[2], c[3], c[4])
    elseif n == 3 then
        self:diffuselowerleft(c[1], c[2], c[3], c[4])
    elseif n == 4 then
        self:diffuselowerright(c[1], c[2], c[3], c[4])
    else
        self:diffuse(c[1], c[2], c[3], c[4])
    end
end
...
function CheckMod(pn, mod)
    return mod and GAMESTATE:PlayerIsUsingModifier(pn, mod)
end
function SummaryBranch()
    ForceSongAndSteps()
    if not scoreIndex then scoreIndex = 1 end
...
function SelectMusicInit(self)
    ...
    optionIndex = 0;
    GhostDataCache = {};
    for pn = 1, 2 do GhostData(pn, "Cache") end
...
function Gameplay(self)
    ...
    Screen():effectclock('music')
    self:luaeffect('Update')
end

function EvaluationInit(self)
    TimedSet.Class = 2;
    RevertHideBG()
    RevertRateAdjust()
    self:queuecommand('FirstUpdate')
...
function JudgmentInit()
    ...
    if FakeCombo == nil or not FakeCombo then FakeCombo = {0, 0}; end
    ...
    for pn = 1, 2 do
        ...
        for i, v in ipairs(trackedStreams) do judge[pn].Stream[i] = {} end
    end
    ...
    for pn = 1, 2 do
        ...
        if px then
            px = px:GetChild('Judgment'):GetChild('');
            px:aux(pn);
            if ModCustom.JudgmentFont[pn] ~= 1 then
                px:Load(THEME:GetPath(EC_GRAPHICS, '', '_Judgments/' ..
                                          judgmentFontList[ModCustom.JudgmentFont[pn]]))
            end
        end
    end
end
    ...
    for pn = 1, 2 do
        if Player(pn) then
            ...
            if (MaxDP[pn] or -1) > 0 then
                if (GetScore(pn) <= 100 * judge[pn].CurDP / MaxDP[pn] - 0.01) and
                    GetScore(pn) ~= judge[pn].Score then
...
function JudgmentCommand(self, n)
    TrackJudgment(self, n)
    JudgmentTween(self)
end
function HoldCommand(self, n)
    TrackJudgment(self, n)
    HoldTween(self)
end
...
function TrackJudgment(self, j, p)
    ...
    local r = j
    if j == 9 then
        r = 7 -- Large number of ok/ng judgments accounting for columns, set mines to 7 and start holds from 8.
    elseif j > 6 then
        judge[pn].Delay = true
        for i, v in ipairs(holdJudgments) do
            if self == v then
                r = (math.mod(i - 1, 16) + 1) + (NumColumns(pn) * (j - 7)) + (7)
            end -- (Column number, modded by columns per player) + (columns used in this chart, for NG) + (number of judgments in list before 1st ok)
        end
    else
        ...
        if table.getn(judge[pn].Steps) >= v then
            ...
            if not judge[pn].Stream[i][0] then -- New stream.
                if not s or s[2] < judge[pn].Steps[1][1] then -- If new stream starts before old stream ends, continue old stream. Should improve consistancy.
                    table.insert(judge[pn].Stream[i], {
                        judge[pn].Steps[1][1], GAMESTATE:GetSongBeat()
                    })
                end
...
function UpdateStreams(pn)
    if judge[pn].Steps[1] then
        ...
        if b > 4 then -- Saves a little work on most updates, and avoids calculating timing windows in beats when t == 0.
            local w = (judge[pn].Steps[1][3] == 6 and 0) or
                          math.min(
                              b / t *
                                  (JudgeWindow(judge[pn].Steps[1][3]) +
                                      JudgeWindow(5)), 4) -- Miss cancels the offset from a later possible miss. Any other judgment is assumed early to be inclusive. Clamped at 4 beats in case of large jumps.
            if b - w > 4 then
                table.remove(judge[pn].Steps, 1)
                for i, v in ipairs(trackedStreams) do
                    if table.getn(judge[pn].Steps) < v then
                        judge[pn].Stream[i][0] = nil
                    end
                end
            end
        end
...
function UpdateCheck(str, pn)
    if _G[str] and _G[str].State and _G[str].Time then
        if _G[str].State[pn] ~=
            (_G[str].Time[pn] ~= _G[str][pn]:GetSecsIntoEffect()) then
            BM(str .. 'P' .. pn)
        end -- State is going to change, but has not changed yet.
...
function UpdateGhostData(pn) -- Keep tap number synced, handle non-taps as they occur.
    while ghost[pn].Data[ghost[pn].I] and (ghost[pn].T < judge[pn].T or
        (ghost[pn].Data[ghost[pn].I][1] > 6 and ghost[pn].Data[ghost[pn].I][2] <
            MusicClock())) do
        local j = ghost[pn].Data[ghost[pn].I][1]
        if j > 7 + NumColumns(pn) then
            j = 8
        elseif j > 7 then
            j = 7
            judge[pn].Delay = true
        elseif j == 7 then
            ...
            ghost[pn].T = ghost[pn].T + 1
        end
        if (ghost[pn].Dead or table.getn(ghost[pn].Data)) > ghost[pn].I then
            ghost[pn].CurDP = ghost[pn].CurDP + ScoreWeight(j)
        end -- Don't update DP count for the deceased.
        ghost[pn][j] = ghost[pn][j] + 1
        ghost[pn].I = ghost[pn].I + 1
        for i, v in ipairs(ghost[pn].Stream) do
            if v[v[0] + 1] and v[v[0] + 1][1] < GAMESTATE:GetSongBeat() then
                v[0] = v[0] + 1
            end
        end
    end
...
function UpdateCompareText(pn) -- upgrade to deal with 'no maxdp' situations? Pure DP comparison or calc DP from score?
    if judge[pn].Delay then
        judge[pn].Delay = nil
        return
    end -- This is used to reduce jitter caused by time quantization of ghost data for holds.
    local n = 0
    if ModCustom.Compare[pn] == 2 then
        if ghost[pn].pn ~= pn then
            n = math.floor(ghost[pn].Score * judge[pn].MaxDP)
        elseif judge[pn].T == ghost[pn].T then
            n = ghost[pn].CurDP
        end
    end
    if ModCustom.Compare[pn] == 3 then
        if ghost[pn].pn ~= 0 then
            n = math.floor(ghost[pn].Score * judge[pn].MaxDP)
        elseif judge[pn].T == ghost[pn].T then
            n = ghost[pn].CurDP
        end
    ...
    if (ModCustom.Compare[pn] == 5 and judge[1].T == judge[2].T) then
        n = judge[math.mod(pn, 2) + 1].CurDP
    end
    if n ~= 0 then
        n = math.max(judge[pn].CurDP - n,
                     GetScore(pn) * MaxDP[pn] / 100 - judge[pn].MaxDP)
        local str = (math.abs(n) > DPLimit and
                        string.format('%+4.2f',
                                      100 * n / MaxDP[pn] + 0.005 * n /
                                          math.abs(n)) .. '%') or
                        string.format('%+1.0f', n) -- convert to percent if ceil positives, floor negatives.
        if math.abs(n) < 0.1 then str = '' end
        compareText[pn]:diffuse(CompareTextColor(n))
        compareText[pn]:settext(str)
    end
end
...
function UpdateMeasureText(pn)
    if ModCustom.Measure[pn] ~= 1 then
        local str = ''
        for i, v in ipairs(trackedStreams) do
            if ModsMaster.Measure.modlist[ModCustom.Measure[pn]] == v then
                local z, k = judge[pn].Stream[i], ghost[pn].Stream[i]
                if z[0] or
                    (table.getn(z) > 0 and judge[pn].Steps[1] and
                        z[table.getn(z)][2] > judge[pn].Steps[1][1]) or v == 0 then -- Streaming, or stream ended in the last measure. Prevents blinking at threshold.
                    if v == 0 then -- Use current beat for total song length, use most recent step for specific note frequencies.
                        ...
                        str = math.floor(
                                  (z[table.getn(z)][2] - z[table.getn(z)][1]) /
                                      4)
                        if str < 2 then str = '' end
                    end
                end
                if tonumber(str) and k and k[k[0]] and k[k[0]][1] <=
                    GAMESTATE:GetSongBeat() then
                    str = str .. '/' ..
                              math.max(
                                  math.floor(
                                      (k[k[0]][2] - z[table.getn(z)][1]) / 4),
                                  str) -- If current stream is longer than recorded, use current length.
                end
            end
        end
...
function CompareText(self)
    ...
    self:zoom(0.4)
    local p = Screen():GetChild('PlayerP' .. pn)
    if p then
        local j = p:GetChild('Judgment')
...
function NumColumns(pn)
    local cols = {
        4, 8, 8, 6, 5, 6, 10, 10, 5, 10, 7, 5, 10, 8, 6, 12, 8, 16, 4, 8, 4, 5,
        8, 8, 10, 5, 9
    }
    local s = GAMESTATE:GetCurrentSteps(pn - 1)
    return s and cols[s:GetStepsType() + 1] or 0
end
...
function GhostData(pn, fnctn, check)
    ...
    local function CompareParams(v) -- Score, MaxDP, Difficulty, Columns, SongDir
        ...
        if f[2] == MaxDP[pn] and f[3] ==
            GAMESTATE:GetCurrentSteps(pn - 1):GetDifficulty() and f[4] ==
            NumColumns(pn) and f[5] == GAMESTATE:GetCurrentSong():GetSongDir() then
            return tonumber(f[1]) or 0
        else
            return false
        end
    end
    ...
    if fnctn == 'Compress' and (Profile(0).Ghost.Save or Profile(pn).Ghost.Save) then
        ...
        for k = 0, pn, pn do -- If data for the song is already saved, find the index. If the new score is higher save in it's place, otherwise don't save.
            ...
            for i, v in ipairs(Profile(k).Ghost) do
                if CompareParams(v) then
                    if CompareParams(v) < GetScore(pn) then
                        slot[k] = i
                    else
                        slot[k] = 0
                    end
        ...
        if judge[pn].Data[1] and (slot[0] ~= 0 or slot[pn] ~= 0) and MaxDP[pn] then -- Only compress if it will be saved somewhere.
            local tab = {'', ''}
            tab[0] = GetScore(pn) .. string.char(1) .. MaxDP[pn] ..
                         string.char(1) ..
                         GAMESTATE:GetCurrentSteps(pn - 1):GetDifficulty() ..
                         string.char(1) .. NumColumns(pn) .. string.char(1) ..
                         GAMESTATE:GetCurrentSong():GetSongDir()
            for k, d in ipairs(judge[pn].Data) do
                local n = string.format('%i', math.max(d[2], 0) * 32 + 1 / 2) *
                              j + d[1] - 1 -- total value
                local t = (n - math.mod(n, z)) / z -- time segments passed before this judgment
                for i = m - 1 - z, 0, -1 do
                    while t >= c + 2 ^ i do
                        c = c + 2 ^ i;
                        tab[1] = tab[1] .. char(m - i)
                    end
                end -- Add time characters as needed.
                tab[1] = tab[1] .. char(n - t * z)
                if k == judge[pn].Dead then
                    tab[1] = tab[1] .. string.char(0)
                end -- Indicate that most recent judgment was fatal.
            end
            for i, v in ipairs(judge[pn].Stream) do
                tab[2] = tab[2] .. (i ~= 1 and string.char(1) or '')
                for j, z in ipairs(v) do
                    tab[2] =
                        tab[2] .. string.char(2) .. '{' .. math.floor(z[1]) ..
                            ',' .. math.floor(z[2]) .. '}'
                end
            end

            if Profile(0).Ghost.Save and slot[0] ~= 0 then
                Profile(0).Ghost[slot[0]] = tab
            end
            if Profile(pn).Ghost.Save and slot[pn] ~= 0 then
                Profile(pn).Ghost[slot[pn]] = tab
            end
    ...
    if fnctn == 'Decompress' then
        ...
        if Player(pn) and
            (ModCustom.Compare[pn] == 2 or ModCustom.Compare[pn] == 3 or
                ModCustom.Measure[pn] ~= 1) then
            local s, b, p = '', {},
                            (ModCustom.Compare[pn] == 3 and {0, pn}) or {pn, 0} -- Order to check for data. Compare score has priority but if its missing use backup for other features.
            for n, k in ipairs(p) do
                if s == '' then
                    for i, v in ipairs(Profile(k).Ghost or {}) do
                        if CompareParams(v) then
                            s = tostring(v[1]);
                            b = ParseString(v[2], string.char(1));
                            ghost[pn].pn = k;
                            ghost[pn].Score = CompareParams(v)
                        end
                    end
                end
            end
            ...
            while string.len(s) > 0 do
                ...
                if n > z then
                    c = c + 2 ^ (m - n)
                elseif n >= 0 then
                    table.insert(ghost[pn].Data, {
                        math.mod(n, j) + 1,
                        (n - math.mod(n, j) + z * c) / j / 32
                    }) -- insert { judgment , seconds*32 }
                else
            ...
            for i, v in ipairs(b) do
                table.insert(ghost[pn].Stream, {});
                ghost[pn].Stream[i][0] = 1
                local a = ParseString(v, string.char(2))
                for j, z in ipairs(a) do
                    if j > 1 then
                        table.insert(ghost[pn].Stream[i],
                                     loadstring('return ' .. z)())
                    end
                end
            end

            if p[1] ~= ghost[pn].pn then
                if p[1] == 0 then
                    ghost[pn].Score =
                        (tonumber(MachineHighScoreText[pn]) or 0) / 100
                else
                    ghost[pn].Score =
                        (tonumber(ProfileHighScoreText[pn]) or 0) / 100
                end
            end
            if ghost[pn].Score == 0 then
                ghost[pn].pn = -1;
                ghost[pn].Score = 1
            end -- If compared score is 0, use subtractive instead.
        end
    end
...
function JudgeGraphArray()
    ...
    for pn = 1, 2 do
        for j = 0, 6 do
            judgeGraphArray[pn][j] = {};
            judgeGraphArray[pn][j][0] = 1
            for i = 1, judgeGraphWidth do
                judgeGraphArray[pn][j][i] = 0
            end
        end
        if judge[pn].Data[1] then
            for k, d in ipairs(judge[pn].Data) do
                local i = math.min(math.floor(
                                       1 + judgeGraphWidth * d[2] /
                                           judge[pn].Data[table.getn(
                                               judge[pn].Data)][2]),
                                   judgeGraphWidth)
                local j = d[1]
                if j <= 6 then -- tap judgments only
                    ...
                    judgeGraphArray[pn][j][0] = math.max(
                                                    judgeGraphArray[pn][j][0],
                                                    judgeGraphArray[pn][j][i])
                end
            end
        end
    end
...
function JudgeGraphCondition()
    judgeFrame = judgeFrame + 1
    if judgeGraphArray[n][j][judgeFrame] ~= 0 then return true end
end

function JudgeGraph(self)
    local t = (judgeGraphHeight - 1) * judgeGraphArray[n][j][judgeFrame] /
                  judgeGraphArray[n][0][judgeFrame]
...
function AddScoreToListFromEval()
    ...
    for pn = 1, 2 do
        if Player(pn) then
            for i = 1, 9 do
                table.insert(judge[pn], _G[judgmentList[i]][pn])
            end
            score[pn] = GetScore(pn)
            fail[pn] = STATSMAN:GetCurStageStats():GetPlayerStageStats(pn - 1)
                           :GetGrade() == GRADE_FAILED
...
function InitializeMods()
    ...
    TimedSet.Reset()
    LoadFromProfile()
    if Profile(0).TimedSets and not GAMESTATE:IsEventMode() then
    ...
    for pn = 1, 2 do
        if Player(pn) then
            ...
            for s, v in pairs(ModCustom) do
                v[pn] = tonumber(t.Mods[s]) or 1
            end
            for i, v in ipairs(judgmentFontList) do
                if t.Mods.JudgmentFont == v then
                    ModCustom.JudgmentFont[pn] = i
                end
            end
            if vocalize and Profile(pn).Voice then
                vocalize[pn] = Profile(pn).Voice
            end
...
function OptionFromEvalPlayerOptions(pn, m)
    ...
    local mods = string.lower(Screen():GetChild('PlayerOptionsP' .. pn)
                                  :GetText())
...
function CapturePlayerMetric(self, str)
    if _G[str][1] or not Player(1) then
        _G[str][2] = self;
        self:aux(2)
    else
        _G[str][1] = self;
...
function JudgeWindow(j)
    local names = {
        'Marvelous', 'Perfect', 'Great', 'Good', 'Boo', 'Miss', 'OK', 'NG',
        'HitMine'
    }
    return GetPref('JudgeWindowSeconds' .. names[j])
end
function ScoreWeight(j)
    local names = {
        'Marvelous', 'Perfect', 'Great', 'Good', 'Boo', 'Miss', 'OK', 'NG',
        'HitMine'
    }
    return GetPref('PercentScoreWeight' .. names[j])
end
function MaxScoreWeight(j)
    if j == 9 then return 0 end
    if j > 6 then return ScoreWeight(7) end
    return ScoreWeight(1)
end
paneItemListDance = {
    'SongNumStepsText', 'SongJumpsText', 'SongHoldsText', 'SongRollsText',
    'SongMinesText', 'SongHandsText', 'MachineHighScoreText',
    'ProfileHighScoreText', 'MachineHighNameText'
}
paneItemListCourse = {
    'CourseNumStepsText', 'CourseJumpsText', 'CourseHoldsText',
    'CourseRollsText', 'CourseMinesText', 'CourseHandsText',
    'CourseMachineHighNameText', 'CourseMachineHighScoreText',
    'CourseProfileHighScoreText'
}
judgmentList = {
    'MarvelousNumber', 'PerfectNumber', 'GreatNumber', 'GoodNumber',
    'BooNumber', 'MissNumber', 'HoldsText', 'RollsText', 'MinesText',
    'JumpsText', 'HandsText'
}
Difficulty = {}
Meter = {}
...
function CapturePane()
    if GAMESTATE:IsCourseMode() then
        paneItemList = paneItemListCourse
    else
        paneItemList = paneItemListDance
    end
    for pn = 1, 2 do
        if Player(pn) then
            for i, v in ipairs(paneItemList) do
                _G[v][pn] = Screen():GetChild('PaneDisplayP' .. pn):GetChild('')
                                :GetChild(v):GetText()
                if i == 7 or i == 8 then
                    _G[v][pn] = string.gsub(_G[v][pn], '%%', '')
                end
            end
            if tonumber(_G[paneItemList[1]][pn]) then
                MaxDP[pn] = _G[paneItemList[1]][pn] * ScoreWeight(1) +
                                _G[paneItemList[3]][pn] * ScoreWeight(7) +
                                _G[paneItemList[4]][pn] * ScoreWeight(7)
            else
                MaxDP[pn] = nil
            end
        end
    ...
    for pn = 1, 2 do
        if Player(pn) then
            for i, v in ipairs(judgmentList) do
                ...
                if j then _G[v][pn] = j:GetText() end
            end
        end
    end
end
...
function CaptureMeter()
    for pn = 1, 2 do
        if Player(pn) then
            ...
            if s then
                Difficulty[pn] = s:GetDifficulty()
            else
                Difficulty[pn] = Screen():GetChild('MeterP' .. pn):GetChild(
                                     'Difficulty'):GetText()
            end
            for i = 0, 5 do
                if DifficultyToThemedString(i) == Difficulty[pn] or
                    string.upper(DifficultyToThemedString(i)) == Difficulty[pn] then
                    Difficulty[pn] = i
                    break
                end
            end
            Meter[pn] = Screen():GetChild('MeterP' .. pn):GetChild('Meter')
                            :GetText()
        end
    end
end
...
function CaptureSteps()
    for pn = 1, 2 do
        if GAMESTATE:GetCurrentSteps(pn - 1) then
            st = GAMESTATE:GetCurrentSteps(pn - 1):GetStepsType()
            break
        end
    end
    steps = {}
    if GAMESTATE:GetCurrentSong() then
        ...
        for n = 1, table.getn(steps) do
            ...
            for i, a in ipairs(steps) do
                local b = steps[m]
                if a:GetDifficulty() < b:GetDifficulty() or
                    (a:GetDifficulty() == b:GetDifficulty() and
                        (a:GetMeter() < b:GetMeter() or
                            (a:GetMeter() == b:GetMeter() and a:GetDescription() <
                                b:GetDescription()))) then m = i end
            end
...
function GetScore(pn)
    ...
    local s = 0
    if Screen():GetChild('PercentP' .. pn) then
        s = Screen():GetChild('PercentP' .. pn):GetChild('PercentP' .. pn)
                :GetText()
    end
    if Screen():GetChild('ScoreP' .. pn) then
        s = Screen():GetChild('ScoreP' .. pn):GetChild(
                'ScoreDisplayPercentage Percent'):GetChild('PercentP' .. pn)
                :GetText()
    end
    s = string.gsub(s, '%%', '')
...
function BPMlabelRate(self)
    s = AdjustedBPM() .. ' BPM ' .. RateModAppend()
    if self then
        self:settext(s)
    else
        return s
...
function RateModText(self)
    s = ''
    if modRate ~= 1 then
        s = string.format('%01.1f', modRate) .. 'x Music Rate'
    end
    if self then
        self:settext(s)
    else
...
function AdjustedBPM(self)
    ...
    if tonumber(s) then
        ...
        if tonumber(bpm[2]) then
            s = s .. '-' .. math.floor(bpm[2] * modRate + 0.5)
        end
    end
    if self then
        self:settext(s)
...
function DisplaySpeedMod(pn)
    ...
    if modType[pn] == 'x' and tonumber(bpm[1]) then
        ...
        if tonumber(bpm[2]) then
            s = s .. '-' ..
                    math.floor(modSpeed[pn] / 100 * bpm[2] * modRate + 0.5)
        end
        s = ' (' .. s .. ')'
    end
...
baseSpeed = {
    "C700", "C800", "C900", "C1000", "C1100", "C1200", "C1300", "C1400", "1x",
    "2x", "3x", "4x", "5x", "6x", "7x", "C400", "C500", "C600"
}
extraSpeed = {
    "0", "+C10", "+C20", "+C30", "+C40", "+C50", "+C60", "+C70", "+C80", "+C90",
    "+.75x", "+.50x", "+.25x"
}
...
rateMods = {
    "1.0x", "1.1x", "1.2x", "1.3x", "1.4x", "1.5x", "1.6x", "1.7x", "1.8x",
    "1.9x", "2.0x"
}
rateModsEdit = {
    "1.0x", "1.1x", "1.2x", "1.3x", "1.4x", "1.5x", "1.6x", "1.7x", "1.8x",
    "1.9x", "2.0x", "0.3x", "0.4x", "0.5x", "0.6x", "0.7x", "0.8x", "0.9x"
}
...
ModsPlayer = {}
ModsMaster = {}
ModsMaster.Perspective = {
    modlist = {'Overhead', 'Hallway', 'Distant', 'Incoming', 'Space'},
    Select = 1
}
ModsMaster.NoteSkin = {modlist = NOTESKIN:GetNoteSkinNames(), Select = 1}
ModsMaster.Turn = {
    modlist = {'Mirror', 'SoftShuffle', 'SmartBlender', 'Blender'},
    default = 'no mirror,no left,no right,no shuffle,no supershuffle,no softshuffle, no spookyshuffle, no smartblender',
    mods = {'mirror', 'softshuffle', 'smartblender', 'supershuffle'}
}
ModsMaster.Hide = {
    modlist = {'Hide Targets', 'Hide Judgments', 'Hide Background'},
    default = 'no dark,no blind,no cover',
    mods = {'dark', 'blind', 'cover'}
}
ModsMaster.Accel = {
    modlist = {'Accel', 'Decel', 'Wave', 'Boomerang', 'Expand', 'Bump'},
    default = 'no boost,no brake,no wave,no boomerang,no expand,no bumpy',
    mods = {'Boost', 'Brake', 'Wave', 'Boomerang', 'Expand', 'Bumpy'}
}
ModsMaster.Scroll = {
    modlist = {'Reverse', 'Split', 'Alternate', 'Cross', 'Centered'},
    default = 'no reverse,no split,no alternate,no cross,no centered'
}
ModsMaster.Effect = {
    modlist = {'Drunk', 'Dizzy', 'Flip', 'Invert', 'Tornado', 'Tipsy', 'Beat'},
    default = 'no drunk,no dizzy,no flip,no invert,no tornado,no tipsy,no beat, no big',
    mods = {'drunk', 'dizzy', 'flip', 'invert', '60% tornado', 'tipsy', 'beat'}
}
ModsMaster.Appearance = {
    modlist = {'Sudden', 'Hidden', 'Blink', 'Stealth'},
    default = 'no hidden,no sudden,no blink,no stealth'
}
ModsMaster.Handicap = {
    modlist = {
        'No Mines', 'No Rolls', 'No Holds', 'No Hands', 'No Jumps', 'No Stretch'
    },
    default = 'no nomines,no noholds,no norolls,no nohands,no nojumps,no nostretch',
    mods = {'nomines', 'norolls', 'noholds', 'nohands', 'nojumps', 'nostretch'}
}
ModsMaster.InsertTaps = {
    name = 'Insert',
    modlist = {'Little', 'Big', 'Quick', 'Skippy', 'Echo', 'Wide', 'Stomp'},
    default = 'no little,no big,no quick,no skippy,no echo,no stomp,no wide',
    mods = {'Little', 'Big', 'Quick', 'Skippy', 'Echo', 'Wide', 'Stomp'}
}
ModsMaster.InsertOther = {
    name = 'Other',
    modlist = {'Planted', 'Floored', 'Twister', 'Mines'},
    default = 'no planted,no floored,no twister,no mines'
}
...
function SetPlayerOptionFlags()
    ...
    for i, v in ipairs(optionsList) do
        if ModsMaster[v] and ModsMaster[v].float then f = '' end
    end
    playerOptions.Flags = playerOptions[optionIndex].Flags or f
end

function LineNames()
    optionIndex = tonumber(optionIndex) and
                      math.mod(optionIndex, table.getn(playerOptions)) + 1 or
                      optionIndex
    optionsList = {}
    lineNames = ''
    ...
    if table.getn(playerOptions) > 1 and optionIndex ~= 'Edit' then
        table.insert(optionsList, 'Next')
        lineNames = lineNames .. ',' .. 'Mod'
    else
        nextScreen = ScreenList('Gameplay')
    end
end
...
function OptionFromList()
    ...
    if ModsMaster[mod].fnctn then
        t = _G[ModsMaster[mod].fnctn](ModsMaster[mod].arg)
    elseif ModsMaster[mod].float then
        t = OptionFloat(mod)
    elseif ModsMaster[mod].modlist then
        t = OptionList(mod)
    else
        t = OptionBool(mod)
    end
...
function OptionFloat(mod)
    ...
    local function move(pn, dir, cnt)
        ModsPlayer[mod][pn + 1] = AddSnap(ModsPlayer[mod][pn + 1], dir, cnt,
                                          {1, 5, 25})
...
function OptionList(mod)
    local Select = string.find(playerOptions.Flags, 'toggle') and
                       ModsMaster[mod].Select ~= 1
    local mods = {};
    ...
    local t = OptionRowBase(ModsMaster[mod].name or mod)
    t.Choices = {}
    for i, v in ipairs(ModsMaster[mod].modlist) do table.insert(t.Choices, v) end
    ...
    t.SaveSelections = function(self, list, pn)
        ...
        for i, v in ipairs(list) do
            if v then ApplyMod(mods[i], pn + 1) end
        end
    end
    return t
end
...
function CustomMod(name, modVar, choices)
    ...
    t.LoadSelections = function(self, list, pn)
        list[ModCustom[modVar][pn + 1]] = true
    end
    t.SaveSelections = function(self, list, pn)
        for i, v in ipairs(list) do
            if v then ModCustom[modVar][pn + 1] = i end
        end
    end
    return t
end
...
function BoolPrefRow(name, pref, tab)
    ...
    t.LoadSelections = function(self, list, pn)
        list[1] = not GetPref(pref)
        list[2] = GetPref(pref)
    end
    t.SaveSelections = function(self, list, pn)
        SetPref(pref, list[2])
        for i, p in ipairs(tab or {}) do SetPref(p, list[2]) end
    end
    return t
end
...
function SliderOption(name, move, display, share)
    ...
    if THEMED_TITLES then
        SliderDisplayFunction[THEME:GetMetric("OptionTitles", name)] = display
    end
    ...
    t.SaveSelections = function(self, list, pn)
        ...
        if Clock(slider[pn + 1][3]) < 0.1 then
            slider[pn + 1][2] = math.min(slider[pn + 1][2] + 1)
        else
            slider[pn + 1][2] = 1
        end
        slider[pn + 1][3] = Clock()
...
function SpeedType()
    local t = OptionRowBase((optionIndex == 'Edit' and 'Speed') or
                                'Speed Mod Type', {'x', 'C', 'm'})
    t.LoadSelections = function(self, list, pn)
        for i, v in ipairs(self.Choices) do
            if modType[pn + 1] == v then list[i] = true end
        end
    end
    t.SaveSelections = function(self, list, pn)
        for i, v in ipairs(list) do
            if v then modType[pn + 1] = self.Choices[i] end
        end
        SetSpeedMod(pn + 1)
...
function SpeedNumber()
    ...
    local function move(pn, dir, cnt)
        modSpeed[pn + 1] = clamp(AddSnap(modSpeed[pn + 1], dir, cnt,
                                         {5, 25, 100}), speedMin, speedMax);
        SetSpeedMod(pn + 1)
    end
...
function MetaMods(s, iRow)
    ...
    t.LoadSelections = function(self, list, pn)
        for i, v in ipairs(metaModsRow.mods) do list[i] = CheckMod(pn, v) end
    end
    ...
    t.SaveSelections = function(self, list, pn)
        ...
        for i, v in ipairs(list) do
            if v then ApplyMod(metaModsRow.mods[i], pn + 1) end
        end
    end
...
function MetaMods1(s) return MetaMods(s, 1) end

function MetaMods2(s) return MetaMods(s, 2) end

function MetaMods3(s) return MetaMods(s, 3) end

function NextScreenOption()
    local t = OptionRowBase('Next Screen',
                            {'Gameplay', 'Select Music', 'More Options'})
    t.OneChoiceForAllPlayers = true
...
function EnableGhostData(a) -- Use an argument of 0 for the operator menu option, to affect machine profile.
    ...
    t.LoadSelections = function(self, list, pn)
        if not Profile(a or pn + 1).Ghost then
            Profile(a or pn + 1).Ghost = {}
        end
        list[2] = Profile(a or pn + 1).Ghost.Save;
        list[1] = not list[2]
    end
    t.SaveSelections = function(self, list, pn)
        Profile(a or pn + 1).Ghost.Save = list[2]
    end
    if a then
        CheckProfile.Ghost = {Save = Profile(0).Ghost and Profile(0).Ghost.Save}
    end
    return t
end
...
function SessionTimer()
    ...
    local function display(text)
        text:settext(SecondsToMSS(Profile(0).SessionTimer))
    end
    local function move(pn, dir, cnt)
        Profile(0).SessionTimer = math.max(
                                      AddSnap(Profile(0).SessionTimer, dir, cnt,
                                              {1, 15, 60, 300}), 0)
    end
...
function CutOffTime()
    ...
    local function display(text)
        text:settext(SecondsToMSS(Profile(0).CutOffTime))
    end
    local function move(pn, dir, cnt)
        Profile(0).CutOffTime = math.max(
                                    AddSnap(Profile(0).CutOffTime, dir, cnt,
                                            {1, 15, 60, 300}), 0)
    end
...
function JudgmentOption()
    return CustomMod('Judgment Font', 'JudgmentFont', judgmentFontList)
end
function LifeBarOption()
    return CustomMod('Life Bar Type', 'LifeBar', {'Normal', 'Surround'})
end
function CompareOption()
    local t = CustomMod('Compare Score', 'Compare',
                        {'None', 'Personal', 'Machine', 'Subtractive'})
    if Player(1) and Player(2) and GAMESTATE:GetCurrentSteps(0) ==
        GAMESTATE:GetCurrentSteps(1) then table.insert(t.Choices, 'Opponent') end
    for pn = 1, 2 do
        if ModCustom.Compare[pn] > table.getn(t.Choices) then
            ModCustom.Compare[pn] = 2
        end
    end
    return t
end
function MeasureOption()
    ...
    for i, v in ipairs(ModsMaster.Measure.modlist) do
        if i > 2 then
            if v == 32 or v == 192 then
                table.insert(t.Choices, v .. 'nds')
            else
                table.insert(t.Choices, v .. 'ths')
            end
        end
    end
    return t
...
function Merciful()
    return BoolPrefRow('Merciful', 'MercifulBeginner',
                       {'FailOffInBeginner', 'FailOffForFirstStageEasy'})
end
...
function RevertRateAdjust()
    for pn = 1, 2 do if modSpeed then ApplyMod(SpeedString(pn), pn) end end
end

function SurroundLife()
    for pn = 1, 2 do
        if ModCustom.LifeBar and ModCustom.LifeBar[pn] == 2 then
            ...
            meter:y(SCREEN_CENTER_Y + lifeBarSizeAdd.OffsetY)
            meter:zoomy((SCREEN_HEIGHT + lifeBarSizeAdd.Height) / height)
            if GAMESTATE:PlayerUsingBothSides() or
                (GAMESTATE:GetNumPlayersEnabled() == 1 and GetPref('SoloSingle')) then
                meter:x(SCREEN_CENTER_X)
                meter:zoomx((SCREEN_WIDTH + lifeBarSizeAdd.Width) / width)
                lifeNormal[pn]:Load(ThemeFile('doublebar.png'));
                lifeNormal[pn]:diffuse(0.2, 0.2, 0.2, 1)
                lifeHot[pn]:Load(ThemeFile('doublebar.png'));
                lifeHot[pn]:diffuse(0.2, 0.2, 0.2, 1)
            else
                meter:x(SCREEN_CENTER_X +
                            ((SCREEN_WIDTH * 320 / 640) + lifeBarSizeAdd.OffsetX) *
                            (pn - 1.5))
                meter:zoomx((SCREEN_WIDTH + lifeBarSizeAdd.Width) / 2 / width)
                lifeNormal[pn]:Load(ThemeFile('white.png'));
                lifeNormal[pn]:diffuse(0.2, 0.2, 0.2, 1)
                lifeHot[pn]:Load(ThemeFile('white.png'));
                lifeHot[pn]:diffuse(0.2, 0.2, 0.2, 1)
                if pn == 1 then
                    lifeNormal[pn]:fadebottom(0.8)
                    lifeHot[pn]:fadebottom(0.8)
                else
                    lifeNormal[pn]:fadetop(0.8)
                    lifeHot[pn]:fadetop(0.8)
                end
            end
        end
    end
...
function RevertHideBG()
    modCover = {}
    for pn = 1, 2 do
        if Player(pn) then
            if CheckMod(pn - 1, 'cover') then modCover[pn] = true end
            local t = Profile(pn)
            if not t.Mods then t.Mods = {} end
            t.Mods.Cover = modCover[pn]
            ApplyMod('no cover', pn)
        end
    end
end
function ApplyHideBG()
    for pn = 1, 2 do if modCover[pn] then ApplyMod('cover', pn) end end
end
...
function DifficultyList()
    ...
    if not twoDifficultyListRows and GAMESTATE:GetNumPlayersEnabled() == 2 and q >
        r then
        listPointerY[1] = math.max(math.min(math.ceil(
                                                (r + listPointer[1] -
                                                    listPointer[2]) / 2), b), a)
        listPointerY[2] = math.max(math.min(math.ceil(
                                                (r + listPointer[2] -
                                                    listPointer[1]) / 2), b), a)
        if listPointer[1] + listPointer[2] < r + 2 and listPointer[1] <= b and
            listPointer[2] <= b then
            listPointerY[1] = listPointer[1];
            listPointerY[2] = listPointer[2]
        end
        if listPointer[1] + listPointer[2] > 2 * (q + 1) - (r + 2) and
            listPointer[1] >= (q + 1) - b and listPointer[2] >= (q + 1) - b then
...
function DifficultyListRow(self, k, t, pn)
    ...
    if not GAMESTATE:GetCurrentSong() then
        if t == 'difficulty' then
            if k - 1 < 5 then
                self:settext(string.upper(DifficultyToThemedString(k - 1)))
                self:diffuse(DifficultyColorRGB(k - 1))
            else
                self:settext('')
            end
        end
        ...
        if t == 'feet' then
            self:zoomy(z)
            self:zoomx(n * z)
            self:customtexturerect(0, 0, n, 1 / 8)
        end
    elseif FixedDifficultyRows() then
        ...
        if s then
            ...
            if t == 'feet' then
                ...
                self:customtexturerect(0, k / 8, math.min(s:GetMeter(), m),
                                       (k + 1) / 8)
            end
            if Song.GetUnlockMethod then
                if t and
                    GAMESTATE:GetCurrentSong()
                        :GetUnlockMethod(s:GetDifficulty()) ~= '' then
                    self:glowshift()
            ...
            if t == 'difficulty' then
                ...
                self:diffuse(DifficultyColorRGB())
            end
            if t == 'meter' then
                self:settext('')
                self:diffuse(DifficultyColorRGB(k - 1))
        ...
        if s then
            if t == 'difficulty' then
                if s:GetDifficulty() < 5 then
                    self:settext(string.upper(
                                     DifficultyToThemedString(s:GetDifficulty())))
                else
                    self:settext(s:GetDescription())
                end
...
function FixedDifficultyRows()
    ...
    for i = 1, l - 1 do
        for j = i + 1, l do
            if steps[i]:GetDifficulty() == steps[j]:GetDifficulty() then
                return false
            end
        end
    end
...
function FrameCapture(self) -- I had a simpler version which checked parent, but on ITG machines parent is not an argument of propagated commands.
    ...
    for j, v in ipairs({'More', 'DisqualifiedP1', 'DisqualifiedP2'}) do
        if Screen():GetChild('Frame'):GetChild(v) == self then return end
    end
    ...
    elseif captureIndex == 2 then -- cursor sprites, title and item text, and underlines
        ...
        elseif IsType(self, 'ActorFrame') and self:GetNumChildren() == 3 then
            table.insert(optionUnderlineSprite,
                         {Row = table.getn(optionRowText)})
            self:propagate(1)
        end
    ...
    elseif captureIndex == 3 then -- underline sprites
        ...
        for j, v in ipairs(optionUnderlineSprite) do
            if not v[3] then
                table.insert(v, self)
                if v[3] then InitializeOptionRow(j) end
                return
            end
        end
    end
...
function InitializeOptionRow(i) -- If (this is a slider option) and (we have all of its underlines) then run the Set underlines, text, and cursors.
    ...
    if t[2]:GetText() == ' ' and (p > 0 and k == optionUnderlineSprite[p].Row) then
        SetOptionRow(k)
    end
end
...
function SetOptionRow(row, c) -- If c is true this will not adjust cursor size. Used for when changing speed mod type, which alters the text of a non-selected row.
    ...
    if type(row) == 'string' then
        ...
        if THEMED_TITLES then
            r = optionRowTextCache[THEME:GetMetric("OptionTitles", name)]
        end
    else
    ...
    for pn = 1, 2 do
        if Player(pn) then
            local p = PlayerIndex(pn) -- Must always check all players. When an option changes the entire row has its text set, not just the changing player.
            if not optionRowText[r][p + 1] then
                optionRowText[r][p + 1] = optionRowText[r][p]
            end -- false when "One Choice for all players"
            display(optionRowText[r][p + 1], pn)
            if not c and optionCursor[p]:GetY() > (optionRow[r - 1]:GetY() or 0) and
                optionCursor[p]:GetY() < (optionRow[r + 1]:GetY() or 0) then
                Size(optionCursorSprite[p], optionRowText[r][p + 1])
            end
            for i, u in ipairs(optionUnderlineSprite) do
                if u.Row == r then
                    Size(optionUnderlineSprite[i - 1 + p],
                         optionRowText[r][p + 1])
                    break
                end
            end
...
function CancelAll(self)
    ...
    for s, v in pairs(ModsPlayer) do
        if tonumber(v[pn]) then
            v[pn] = 0
        else
            v[pn] = nil
        end
...
function MenuTimerSeconds(s)
    TimedSet.MenuTimer = s;
    if IsTimedSet() or not GetPref('MenuTimer') then
        return -1
    else
        return s
    end
...
function IsTimedSet()
    return not GAMESTATE:IsCourseMode() and Profile(0).TimedSets
end
